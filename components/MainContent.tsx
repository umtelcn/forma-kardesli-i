    ...
    try {
      const [teamsRes, totalsRes, donationsRes] = await Promise.all([
        ...
        supabase
          .from('donations')
          .select(
            'created_at, type, quantity, amount_tl, teams(name, logo_url), donors(display_name, identity_type)'
          )
  ...
  const tabs = [
    {
      ...
      label: 'Forma Bağışla',
      icon: faGift,
      description: 'Çocuklara forma hediye edin',
      color: 'emerald',
    },
    {
      id: 'donations',
      label: 'Bağış Listesi',
      icon: faListCheck,
      description: 'Son yapılan bağışları görüntüleyin',
      color: 'blue',
    },
    {
      id: 'happiness',
      label: 'Mutluluk Galerisi',
      icon: faImages,
      description: 'Çocukların sevinç dolu anları',
      color: 'purple',
    },
  ];
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        {/* Sekme İçeriği Başlığı */}
        <div className="mt-6 mb-4">
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-xl bg-gradient-to-br ${
                getColorClasses(
                  tabs.find((t) => t.id === currentTab)?.color || 'emerald'
                ).gradient
              } shadow-sm`}
            >
              <FontAwesomeIcon
                icon={tabs.find((t) => t.id === currentTab)?.icon || faGift}
    ...
    <div className="text-center py-16 bg-gradient-to-br from-white via-gray-50 to-purple-50/30 rounded-3xl shadow-lg border border-gray-100 relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-gradient-to-br from-purple-400 to-purple-600 shadow-xl shadow-purple-500/25 mb-8">
          <FontAwesomeIcon
            icon={faImages}
            className="text-white text-4xl drop-shadow-sm"
          />
        </div>
